...
def is_leaf(x, left_child, right_child):
...
def flatten(root, transformer, left_child, right_child):
    ...
    def recurse(x):
        if is_leaf(x, left_child, right_child):
...
def preorder_indexes(root, left_child, right_child, idx=1):
    ...
    left_subtree = preorder_indexes(left_child(root), left_child, right_child,
                                     idx=idx+1)
    
    max_index_in_left = rightmost(left_subtree)
    right_subtree = preorder_indexes(right_child(root), left_child, right_child,
